...
def recons(m_t, DO=1, L=1.0, vertical=False, t=0.3):
    ...
    return x_est, stripes, x_est2


def channel_norm(img):
    min_val = np.min(img, axis=(0, 1), keepdims=True)
    max_val = np.max(img, axis=(0, 1), keepdims=True)
    return (img - min_val) / (max_val - min_val + 1e-4)


def custom_tone(img):
    hdr = channel_norm(img)
    ...
    tmo = cv2.createTonemapReinhard(intensity=0.0, light_adapt=0.2, color_adapt=0.0)
